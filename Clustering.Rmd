...
difs <- difs %>% select(lab_1, lab_2, lab_3, lab_4, lab_5, primeira_prova, ultima_prova, projeto) %>% na.omit()
set.seed(123)
explorando_k = tibble(k = 1:15) %>%
    mutate(agrupamento = map(k, ~ kmeans(
...
evol <- agrupado %>% select(lab_1, lab_2, lab_3, primeira_prova, lab_4, lab_5, ultima_prova, projeto, .cluster)
